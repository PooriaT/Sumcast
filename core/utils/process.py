...
def process_podcast_text(podcast_name: str, episode_name: str, client_id: str) -> None:
    ...
    if podcast_url:
        ...
        if episode_audio_url and "Failed to fetch data" not in episode_audio_url:
            ...
            whole_text = transcribe_large_audio_with_whisper(f"{BASE_AUDIO_PATH}podcast_{client_id}.mp3", client_id)
            with open(os.path.join(BASE_TEXT_PATH, f"whole_text_{client_id}.txt"), "w", encoding="utf-8") as file:
                file.write(whole_text)
            print("Podcast Text generated successfully!")
            summary = summarizing(os.path.join(BASE_TEXT_PATH, f"whole_text_{client_id}.txt"), streaming=False)
